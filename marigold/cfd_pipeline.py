...
class CFDFieldOutput(BaseOutput):
    """
...
class CFDiffPipleline(DiffusionPipeline):
    """
    ...
    def __call__(
        ...
        duplicated_flow = flow_in.expand(ensemble_size, -1, -1, -1)
        single_rgb_dataset = TensorDataset(duplicated_rgb, duplicated_flow)
        ...
        if show_progress_bar:
            iterable = tqdm(
                single_loader, desc=" " * 2 + "Inference batches", leave=False
            )
        else:
            iterable = single_loader
        
        for batch in iterable:
            batched_img, batch_flow = batch
            # text_in = text_in.expand(_bs)
    ...
    def single_infer(
        ...
        Args:
            rgb_in (`torch.Tensor`):
                Input RGB image.
            text_in: (`str`):
                Input text prompt as condition.
            num_inference_steps (`int`):
        ...
        # Encode flow
        flow_in = flow_in.repeat(1, 3, 1, 1).to(device) # B, 3, H, W
        flow_latent = self.encode_rgb(flow_in) # B, 4, H', W'
        ...
        text_embed = text_embed.repeat((rgb_latent.shape[0], 1, 1)).to(device)  # [B, 2, 1024]
        ...
        for i, t in iterable:
            unet_input = torch.cat(
                [rgb_latent, flow_latent, depth_latent], dim=1
