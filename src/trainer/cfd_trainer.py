...
class CFDTrainer:
    def __init__(
        ...
        self.model: CFDiffPipleline = model
        self.device = device
    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in skip_first_batches(self.train_loader, self.n_batch_in_epoch):
                ...
                # Get data
                rgb = batch["inputs"].to(device)
                flow = batch["flows"].to(device)
                gt_for_latent = batch["outputs"].to(device)
                prompt = batch["prompt"]
                
                batch_size = rgb.shape[0]

                with torch.no_grad():
                    rgb_latent = self.model.encode_rgb(rgb)   # Encode image: [B, 4, h, w]
                    flow_latent = self.encode_depth(flow) # Encode flow: [B, 4, h, w]
                    ...
                    text_embed = self.model.text_encoder(text_input_ids)[0].to(self.model.dtype)
                ...
                cat_latents = torch.cat(
                    [rgb_latent, flow_latent, noisy_latents], dim=1
                )  # [B, 12, h, w]
    ...
    def _train_step_callback(self):
        ...
        # Visualization
        # if self.vis_period > 0 and self.effective_iter % self.vis_period == 0:
        #     self.visualize()
    ...
    def validate(self):
        for i, val_loader in enumerate(self.val_loaders):
            ...
            if 0 == i:
                ...
                if (
                    "minimize" == self.main_val_metric_goal and main_eval_metric < self.best_metric
                    or "maximize" == self.main_val_metric_goal and main_eval_metric > self.best_metric
                ):
    ...
    def validate_single_dataset(
        ...
        for i, batch in enumerate(
            ...
            # Get data
            rgb = batch["inputs"].to(self.device)
            flow = batch["flows"].to(self.device)
            label = batch["label_raw"].to(self.device)
            prompt = batch["prompt"]
            ...
            # Predict depth
            pipe_out: CFDFieldOutput = self.model(
                rgb_norm=rgb,
                flow_in=flow,
                text_in=prompt,
